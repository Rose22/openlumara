...
import yaml
import copy
import openlumara.core as core
import openlumara.modules
import openlumara.user_modules
import openlumara.channels
import pkgutil
import hashlib
import json
import inspect
...
def _get_registry_data(enabled_channels=None, enabled_modules=None, enabled_user_modules=None):
    ...
    available_channels = _discover_available_names(openlumara.channels)
    available_modules = _discover_available_names(openlumara.modules)
    available_user_modules = _discover_available_names(openlumara.user_modules)

    # Only import and instantiate ENABLED items
    chan_inst = list(core.modules.load(
        openlumara.channels, core.channel.Channel, filter=enabled_channels
    )) if enabled_channels else []

    mod_inst = list(core.modules.load(
        openlumara.modules, core.module.Module, filter=enabled_modules
    )) if enabled_modules else []

    user_mod_inst = list(core.modules.load(
        openlumara.user_modules, core.module.Module, filter=enabled_user_modules
...
def _get_module_schema_cache():
    ...
    package_map = {
        "channels": (openlumara.channels, core.channel.Channel),
        "modules": (openlumara.modules, core.module.Module),
        "user_modules": (openlumara.user_modules, core.module.Module)
    }
