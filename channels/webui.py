...
# TODO: remove API status checks from both backend and frontend
# TODO: rewrite frontend to alpine.js
# TODO: remove dead code from backend python (this file)
# TODO: remove dead code from frontend javascript
# TODO: remove duplicate code from backend and frontend
...
def create_webui_app(channel) -> FastAPI:
    """Create and configure the FastAPI app for this channel instance."""
    ...
    # Inject channel into request state for Depends
    @app.middleware("http")
    ...
    class WebSocketManager:
        ...
        async def connect(self, websocket: WebSocket, user: str = "anonymous"):
            ...
            current_chat_id = await channel.context.chat.get_id()
        ...
        async def start_background_stream(self, chat_id: str, generator: Any):
            ...
            stream_cancellations = channel.stream_cancellations
    ...
    channel.websocket_manager = WebSocketManager()
    ...
    async def authenticate_websocket(websocket: WebSocket) -> Optional[str]:
        ...
        if not bool(channel.config.get("require_login")):
            return "anonymous"
        ...
        if session_cookie:
            try:
                import itsdangerous
                signer = itsdangerous.TimestampSigner(channel.config.get("secret_key"))
                import base64
                ...
                channel.log("webui", f"WebSocket session auth failed: {core.detail_error(e)}")

        token = websocket.query_params.get('token')
        if token and token in channel.ACTIVE_TOKENS:
            return "token_user"

        auth_header = websocket.headers.get('authorization', '')
        if auth_header.startswith('Bearer '):
            token = auth_header[7:]
            if token in channel.ACTIVE_TOKENS:
                return "token_user"

        return None

    async def get_current_user(request: Request):
        if not channel or not bool(channel.config.get("require_login")):
            return "user"
    ...
    def get_channel(request: Request) -> "Webui":
        return request.state.channel_instance
    ...
    @app.get("/login")
    async def login_page(request: Request, channel: "Webui" = Depends(get_channel)):
        if not channel or not bool(channel.config.get("username")):
            return RedirectResponse(url='/')
        return templates.TemplateResponse(request, "login.html", {"request": request, "error": None})

    @app.post("/login")
    async def login_post(request: Request, channel: "Webui" = Depends(get_channel)):
        if not channel or not bool(channel.config.get("username")):
            return RedirectResponse(url='/')
        ...
        attempts[:] = [t for t in attempts if now - t < int(channel.config.get("rate_limit_window"))]
        if len(attempts) >= int(channel.config.get("login_max_attempts")):
    ...
    async def api_login(request: Request):
        ...
        if username == expected_username and password == expected_password:
            ...
            channel.ACTIVE_TOKENS.add(token)
            return {'token': token}
        else:
    ...
    async def api_logout(request: Request):
        ...
        if auth_header and auth_header.startswith('Bearer '):
            token = auth_header[len('Bearer '):]
            if token in channel.ACTIVE_TOKENS:
                channel.ACTIVE_TOKENS.remove(token)
        return {'success': True}
    ...
    def get_api_status():
        ...
        status = channel.manager.get_api_status()
        result = {
    ...
    @app.get("/")
    async def index(request: Request, channel: "Webui" = Depends(get_channel)):
        return templates.TemplateResponse(
            ...
            "index.html",
            {
                ...
                "js_files": JS_FILES,
                "css_files": CSS_FILES,
                "require_login": bool(channel.config.get("require_login"))
            }
        )
    ...
    async def get_messages(user: str = Depends(require_auth)):
        ...
        messages_orig = await channel.context.chat.get() or []
        messages = copy.deepcopy(messages_orig)
        current_id = await channel.context.chat.get_id()
    ...
    async def get_messages_since(index: int = 0, user: str = Depends(require_auth)):
        ...
        messages = copy.deepcopy(messages_orig)
        current_id = await channel.context.chat.get_id()
        current_title = await channel.context.chat.get_title()
        current_tags = await channel.context.chat.get_tags() or []
    ...
    async def start_ai_stream_task(channel: "Webui", chat_id: str, payload_body: dict):
        ...
        async def generator():
            ...
            try:
                async for token_data in channel.send_stream(payload_body, commands_authorized=True):
                    if stream_id in channel.stream_cancellations:
                        channel.stream_cancellations.discard(stream_id)
                    ...
                    if isinstance(token_data, dict):
                        if token_data.get('type') == "user_message":
                            try:
                                ...
                                channel.log("webui", f"error while sending user message: {core.detail_error(e)}")
        ...
        await channel.websocket_manager.start_background_stream(chat_id, generator())
        return stream_id
    ...
    async def stream_message(request: Request, user: str = Depends(require_auth)):
        ...
        data = await request.json()
        chat_id = await channel.context.chat.get_id() or "default"
        stream_id = await start_ai_stream_task(channel, chat_id, data)
    ...
    async def send_message(request: Request, user: str = Depends(require_auth)):
        ...
        response = await channel.send(data, commands_authorized=True)
        ...
        await channel.websocket_manager.broadcast({"type": "messages_updated", "messages": messages, "chat_id": current_id})
        await channel.websocket_manager.broadcast({"type": "stream_complete", "buffer": [], "index": next_index})
    ...
    async def edit_message(request: Request, user: str = Depends(require_auth)):
        ...
        if 0 <= index < len(messages):
            if messages[index].get('role') in ('user', 'assistant'):
                ...
                await channel.context.chat.set(messages)
                await channel.websocket_manager.broadcast({"type": "messages_updated", "messages": await channel.context.chat.get()})
    ...
    async def delete_message(request: Request, user: str = Depends(require_auth)):
        ...
        if 0 <= int(index) < len(messages):
            if messages[index].get('role') in ('user', 'assistant', 'command', 'command_response') or messages[index].get('role', '').startswith('announce_'):
                await channel.context.chat.delete_from(index)
                remaining = len(await channel.context.chat.get())
                await channel.websocket_manager.broadcast({"type": "messages_updated", "messages": await channel.context.chat.get()})
    ...
    async def cancel_stream(request: Request, user: str = Depends(require_auth)):
        data = await request.json()
        stream_id = data.get('id')
        channel.manager.API.cancel_request = True
        if stream_id:
            channel.stream_cancellations.add(stream_id)
        return {'success': True}
    ...
    async def upload_file(request: Request, user: str = Depends(require_auth)):
        ...
        await channel.context.chat.add({"role": "user", "content": message_content})
        total = len(await channel.context.chat.get())
    ...
    async def search_chats(request: Request, user: str = Depends(require_auth)):
        ...
        all_chats = await channel.context.chat.get_all()
    ...
    async def load_chat(id: str, user: str = Depends(require_auth)):
        ...
        curr_chat_id = await channel.context.chat.get_id() or ""
        ...
        if not is_current_chat:
            await channel.websocket_manager.broadcast({"type": "chat_switched", "chat_id": loaded_id})
    ...
    async def update_chat_category(request: Request, user: str = Depends(require_auth)):
        ...
        try:
            if not was_current:
                load_response = await channel.context.chat.load(chat_id)
                if not load_response:
                    raise HTTPException(status_code=404, detail="Failed to load chat")

            await channel.context.chat.set_category(new_category)

            if not was_current and current_id:
                await channel.context.chat.load(current_id)
    ...
    async def new_chat(request: Request, user: str = Depends(require_auth)):
        ...
        await channel.context.chat.new(category=data.get('category'), metadata=data.get('metadata'))

        await channel.websocket_manager.broadcast({
            "type": "chat_switched",
            "chat_id": await channel.context.chat.get_id(),
            "buffer": []
        })

        return {
            'success': True, 'chat': {
                'id': await channel.context.chat.get_id(),
    ...
    async def clear_chat(user: str = Depends(require_auth)):
        await channel.context.chat.clear()
    ...
    async def delete_chat(request: Request, user: str = Depends(require_auth)):
        ...
        success = await channel.context.chat.delete(conv_id)
        if success is False:
            raise HTTPException(status_code=404, detail="Chat not found")
    ...
    async def get_all_tags(user: str = Depends(require_auth)):
        ...
        all_chats = await channel.context.chat.get_all() or []
        tags = set()
    ...
    async def update_chat_tags(request: Request, user: str = Depends(require_auth)):
        ...
        await channel.context.chat.set_tags(tags)
    ...
    async def add_chat_tag(request: Request, user: str = Depends(require_auth)):
        ...
        success = await channel.context.chat.add_tag(tag)
    ...
    async def remove_chat_tag(request: Request, user: str = Depends(require_auth)):
        ...
        success = await channel.context.chat.pop_tag(tag)
        return {'success': success, 'tag': tag}
    ...
    async def save_settings(request: Request, user: str = Depends(require_auth)):
        data = await request.json()
        form_data = data.get("settings", data)
        ...
        if changed_modules:
            for module_name in changed_modules:
                try:
                    ...
                    channel.log("webui", f"Error reloading module {module_name}: {core.detail_error(e)}")
            ...
            for filename in filenames:
                ...
                if ext in ['.json', '.yml', '.yaml', '.mp']:
                    try:
                        ...
                        channel.log("webui", f"Error reading {rel_path}: {core.detail_error(e)}")
                        file_type = 'unknown'
                elif ext in ['.txt', '.md']:
                    file_type = 'text'
                else:
    ...
    async def load_storage_file(file: str, user: str = Depends(require_auth)):
        ...
        try:
            ...
            channel.log("webui", f"Error loading storage file: {core.detail_error(e)}")
    ...
    async def save_storage_file(request: Request, user: str = Depends(require_auth)):
        ...
        try:
            ...
            channel.log("webui", f"Saved storage file: {file_path}")
            return {'success': True}

        except Exception as e:
            err_msg = core.detail_error(e) if core.debug else str(e)
            channel.log("webui", f"Error saving storage file: {core.detail_error(e)}")
            raise HTTPException(status_code=500, detail=err_msg)
    ...
    async def delete_storage_key(request: Request, user: str = Depends(require_auth)):
        ...
        try:
            ...
            err_msg = core.detail_error(e) if core.debug else str(e)
            channel.log("webui", f"Error deleting key: {core.detail_error(e)}")
            raise HTTPException(status_code=500, detail=err_msg)
    ...
    async def add_storage_key(request: Request, user: str = Depends(require_auth)):
        ...
        try:
            ...
            err_msg = core.detail_error(e) if core.debug else str(e)
            channel.log("webui", f"Error adding key: {core.detail_error(e)}")
            raise HTTPException(status_code=500, detail=err_msg)
    ...
    async def restart_server(user: str = Depends(require_auth)):
        channel.log("webui", "Restart triggered")
